...
namespace rnoh {
    ...
    TextInputComponentInstance::TextInputComponentInstance(Context context) : CppComponentInstance(std::move(context)) {
    ...
    facebook::react::TextInputMetrics TextInputComponentInstance::getTextInputMetrics(ArkUI_NodeEvent *event) {
        auto textInputMetrics = facebook::react::TextInputMetrics();
        textInputMetrics.contentOffset =
            m_multiline ? m_textAreaNode.getTextAreaOffset() : m_textInputNode.getTextInputOffset();
    ...
    void TextInputComponentInstance::onPropsChanged(SharedConcreteProps const &props) {
        ...
        if (!m_props || props->traits.secureTextEntry != m_props->traits.secureTextEntry ||
            ...
            m_textInputNode.setInputType(props->traits.secureTextEntry
    ...
    void TextInputComponentInstance::onStateChanged(SharedConcreteState const &state) {
        CppComponentInstance::onStateChanged(state);

        std::ostringstream contentStream;
        for (auto const &fragment : state->getData().attributedStringBox.getValue().getFragments()) {
