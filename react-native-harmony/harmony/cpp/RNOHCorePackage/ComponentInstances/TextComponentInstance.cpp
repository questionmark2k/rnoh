...
void TextComponentInstance::onPropsChanged(std::shared_ptr<const facebook::react::ParagraphProps> const &textProps) {
    ...
    DLOG(INFO) << "[text-debug] textProps->setPadding top=" << info.top << ", right=" << info.right
              << ", bottom=" << info.bottom << ", left=" << info.left;
    m_textNode.setPadding(info.top, info.right, info.bottom, info.left);
    
    // Copy Option
    DLOG(INFO) << "[text-debug] textProps->isSelectable=" << textProps->isSelectable;
    ...
    if (maxFontSize.has_value()) {
        ...
        DLOG(INFO) << "[text-debug] maxFontSize=" << maxFontSize.value();
    }
    ...
    if (textProps->rawProps != nullptr) {
        ...
        if (!m_props || stackAlign != stackAlignOld) {
            DLOG(INFO) << "[text-debug] stackAlign=" << stackAlign << ", stackAlignOld=" << stackAlignOld;
        ...
        if (textProps->rawProps.count("disabled") != 0) {
            ...
            DLOG(INFO) << "[text-debug] rawProps->disabled=" << enableFlag;
        ...
        if (minFontSize.has_value()) {
            ...
            DLOG(INFO) << "[text-debug] minFontSize=" << minFontSize.value();
        }
    }
    DLOG(INFO) << "[text-debug] setProps end";
}
...
void TextComponentInstance::onStateChanged(
    ...
    m_children.clear();
    m_childIndex = 0;
    DLOG(INFO) << "[text-debug] getFragments size:" << textState->getData().attributedString.getFragments().size();
    for (auto fragment : textState->getData().attributedString.getFragments()) {
        if (fragment.string != IMAGE_SPAN_FLAG) {
            ...
            DLOG(INFO) << "[text-debug] create span text=" << fragment.string.c_str() << ", index=" << m_childIndex;
            m_textNode.insertChild(*spanNode, m_childIndex);
            m_children.push_back(spanNode);
            setFragment(fragment, spanNode, m_childIndex);
            m_childIndex++;
        } else {
            DLOG(INFO) << "[text-debug] create image span, index=" << m_childIndex;
...
void TextComponentInstance::setTextAttributes(const facebook::react::TextAttributes &textAttributes) {
    ...
    if (textAttributes.alignment.has_value()) {
        ...
        DLOG(INFO) << "[text-debug] textAttributes.alignment=" << align;
...
void TextComponentInstance::setFragment(const facebook::react::AttributedString::Fragment &fragment,
    ...
    DLOG(INFO) << "[text-debug] fontColor=" << (uint32_t)(*textAttributes.foregroundColor);
    if (textAttributes.foregroundColor) {
        spanNode->setFontColor((uint32_t)(*textAttributes.foregroundColor));
    }
    DLOG(INFO) << "[text-debug] fontFamily=" << textAttributes.fontFamily;
    ...
    if (textAttributes.fontWeight.has_value()) {
        ...
        DLOG(INFO) << "[text-debug] realFontWeight=" << realFontWeight << ", index=" << index;
    ...
    LOG(INFO) << "[text-debug] textAttributes.fontSize=" << textAttributes.fontSize << ", fontSize=" << fontSize
              << ", index=" << index;
    spanNode->setFontSize(fontSize);

    // FontStyle
    if (textAttributes.fontStyle.has_value()) {
        DLOG(INFO) << "[text-debug] textAttributes.fontStyle=" << (int32_t)textAttributes.fontStyle.value()
                  << ", index=" << index;
    ...
    if (textAttributes.textDecorationLineType.has_value()) {
        ...
        DLOG(INFO) << "[text-debug] textAttributes.textDecorationLineType="
                          << type
    ...
    LOG(INFO) << "[text-debug] textAttributes.lineHeight=" << (float)textAttributes.lineHeight
              << ", lineHeight=" << lineHeight << ", index=" << index;
    spanNode->setTextLineHeight(lineHeight);

    // BackgroundColor
    DLOG(INFO) << "[text-debug] textAttributes.backgroundColor=" << (uint32_t)(*textAttributes.backgroundColor)
              << ", index=" << index;
    ...
    if (!std::isnan(textAttributes.letterSpacing)) {
        DLOG(INFO) << "[text-debug] textAttributes.letterSpacing=" << textAttributes.letterSpacing
                  << ", index=" << index;
    ...
    DLOG(INFO) << "[text-debug] setTextShadow:" << textShadowRadius << ", textShadowType:" << textShadowType
    ...
    if (textAttributes.textTransform.has_value()) {
        DLOG(INFO) << "[text-debug] textAttributes.textTransform=" << (int)textAttributes.textTransform.value()
                  << ", index=" << index;
    ...
    DLOG(INFO) << "[text-debug] setFragment end, index=" << index;
}
...
void TextComponentInstance::setParagraphAttributes(const facebook::react::ParagraphAttributes &paragraphAttributes) {
    // TextMaxLines
    DLOG(INFO) << "[text-debug] paragraphAttributes.maximumNumberOfLines=" << paragraphAttributes.maximumNumberOfLines;
    ...
    DLOG(INFO) << "[text-debug] paragraphAttributes.heightAdaptivePolicy="
    ...
    DLOG(INFO) << "[text-debug] paragraphAttributes.ellipsizeMode=" << ellipsizeMode;
    if (ellipsizeMode == -1) {
