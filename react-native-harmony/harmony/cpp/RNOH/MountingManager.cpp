...
namespace rnoh {
    ...
    void MountingManager::performMountInstructions(react::ShadowViewMutationList const &mutations,
    ...
    void MountingManager::performTransaction(facebook::react::MountingCoordinator::Shared const &mountingCoordinator) {
        ...
        mountingCoordinator->getTelemetryController().pullTransaction(
            ...
            [this, surfaceId](react::MountingTransaction const &transaction,
                              react::SurfaceTelemetry const &surfaceTelemetry) {
                // Mounting
                performMountInstructions(transaction.getMutations(), surfaceId);
            },
            [this](react::MountingTransaction const &transaction, react::SurfaceTelemetry const &surfaceTelemetry) {
                // Did mount
                this->processMutations(transaction.getMutations());
            });
    }

    void MountingManager::processMutations(facebook::react::ShadowViewMutationList mutations) {
        taskExecutor->runTask(TaskThread::MAIN, [triggerUICallback = this->triggerUICallback, mutations = mutations] {
            triggerUICallback(mutations);
        });
    }

    void MountingManager::dispatchCommand(facebook::react::Tag tag, std::string const &commandName,
