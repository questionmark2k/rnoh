...
RemoteConnection::RemoteConnection(napi_env env, napi_value connection) : m_env(env) {
    napi_create_reference(env, connection, 1, &m_connectionRef);
}

RemoteConnection::~RemoteConnection() {
    mainThreadTaskRunner->runAsyncTask([env = this->m_env, ref = this->m_connectionRef] {
        ArkJS arkJs(env);
...
void RemoteConnection::onMessage(std::string message) {
    mainThreadTaskRunner->runAsyncTask([env = this->m_env, ref = this->m_connectionRef, message = std::move(message)] {
        ArkJS arkJs(env);
...
static napi_value wrapLocalConnection(napi_env env, std::unique_ptr<facebook::react::ILocalConnection> localConnection) {
    ArkJS arkJs(env);
    auto connectionRawPtr = localConnection.release();

    try {
        auto sendMessage = arkJs.createFunction(
            "sendMessage", [](auto env, auto cbInfo) {
                ArkJS arkJs(env);
                facebook::react::ILocalConnection *connection;
                size_t argc = 1;
                napi_value args[1];
                napi_get_cb_info(env, cbInfo, &argc, args, nullptr, (void**)&connection);
                if (argc < 1) {
        ...
        auto disconnect = arkJs.createFunction(
            "disconnect", [](auto env, auto cbInfo) {
                facebook::react::ILocalConnection *connection;
                napi_get_cb_info(env, cbInfo, 0, nullptr, nullptr, (void**)&connection);
...
static napi_value connect(napi_env env, napi_callback_info info) {
    ...
    auto localConnection = ::facebook::react::getInspectorInstance().connect(pageId, std::move(remoteConnection));
...
napi_value getInspectorWrapper(napi_env env, napi_callback_info info) {
    ...
    napi_property_descriptor desc[] = {
        {"getPages", nullptr, getPages, nullptr, nullptr, nullptr, napi_default, nullptr},
