  ...
  public:
    ...
    std::vector<Rects> getRectsForFragments() const {
        ...
        for (size_t i = 0; i < m_fragmentLengths.size(); i++) {
            ...
            auto textBoxes = OH_Drawing_TypographyGetRectsForRange(m_typography.get(), fragmentBegin, fragmentEnd, RECT_HEIGHT_STYLE_MAX, RECT_WIDTH_STYLE_MAX);
            ...
            for (size_t j = 0; j < textBoxCount; j++) {
                ...
                rect.size.width = OH_Drawing_GetRightFromTextBox(textBoxes, j) - OH_Drawing_GetLeftFromTextBox(textBoxes, j);
                rect.size.height = OH_Drawing_GetBottomFromTextBox(textBoxes, j) - OH_Drawing_GetTopFromTextBox(textBoxes, j);
...
class ArkUITypographyBuilder final {
  public:
    ArkUITypographyBuilder(OH_Drawing_TypographyStyle *typographyStyle) : m_typographyHandler(OH_Drawing_CreateTypographyHandler(typographyStyle, OH_Drawing_CreateFontCollection()), OH_Drawing_DestroyTypographyHandler) {}
    ...
    void addFragment(const facebook::react::AttributedString::Fragment &fragment) {
    ...
    ArkUITypography build() const {
        return ArkUITypography(m_typographyHandler.get(), m_attachmentCount, m_fragmentLengths, m_maximumWidth);
    }

  private:
    void addTextFragment(const facebook::react::AttributedString::Fragment &fragment) {
    ...
    void addAttachment(const facebook::react::AttributedString::Fragment &fragment) {
