...
namespace rnoh {
    ...
    public:
        ...
        CustomComponentArkUINodeHandleFactory(napi_env env, napi_ref customRNComponentFrameNodeFactoryRef,
                                              TaskExecutor::Shared taskExecutor)
            : m_env(env), m_taskExecutor(taskExecutor),
        ...
        ArkUI_NodeHandle create(facebook::react::Tag tag, std::string componentName) {
            ...
            m_taskExecutor->runSyncTask(
                TaskThread::MAIN, [=, &arkTSNodeHandle, componentName = std::move(componentName)] {
                    ArkJS arkJs(m_env);
                    auto n_result = arkJs.getObject(m_customRNComponentFrameNodeFactoryRef)
                                        .call("create", {arkJs.createInt(tag), arkJs.createString(componentName)});
                    auto errorCode = OH_ArkUI_GetNodeHandleFromNapiValue(m_env, n_result, &arkTSNodeHandle);
                    if (errorCode != 0) {
