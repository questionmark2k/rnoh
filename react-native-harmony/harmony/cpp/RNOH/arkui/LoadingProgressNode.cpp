...
void LoadingProgressNode::setLoadingProgressNodeColor(
  ...
  maybeThrow(NativeNodeApi::getInstance()->setAttribute(
      m_nodeHandle, NODE_LOADING_PROGRESS_COLOR, &colorItem));
}
...
void LoadingProgressNode::setLoadingProgressNodeAnimating(const bool& enable) {
  ...
  maybeThrow(NativeNodeApi::getInstance()->setAttribute(
      m_nodeHandle, NODE_LOADING_PROGRESS_ENABLE_LOADING, &typeItem));
}
