...
static constexpr ArkUI_NodeEventType TEXT_INPUT_NODE_EVENT_TYPES[] = {NODE_TEXT_INPUT_ON_PASTE,
...
namespace rnoh {
    ...
    TextInputNode::TextInputNode()
        : TextInputNodeBase(ArkUI_NodeType::ARKUI_NODE_TEXT_INPUT), m_childArkUINodeHandle(nullptr) {
    ...
    void TextInputNode::setSelectedBackgroundColor(facebook::react::SharedColor const &color) {
        ArkUI_NumberValue selectedBackgroundColor[] = {{.u32 = rnoh::convertColorToTranslucent(color)}};
        ArkUI_AttributeItem colorItem = {selectedBackgroundColor,
                                         sizeof(selectedBackgroundColor) / sizeof(ArkUI_NumberValue)};
        maybeThrow(NativeNodeApi::getInstance()->setAttribute(m_nodeHandle, NODE_TEXT_INPUT_SELECTED_BACKGROUND_COLOR,
                                                              &colorItem));
    }
    ...
    void TextInputNode::setEnterKeyType(facebook::react::ReturnKeyType returnKeyType) {
        ArkUI_NumberValue value[] = {{.i32 = static_cast<int32_t>(rnoh::convertEnterKeyType(returnKeyType))}};
        ArkUI_AttributeItem item = {value, sizeof(value) / sizeof(ArkUI_NumberValue)};
    ...
    void TextInputNode::setFont(facebook::react::TextAttributes const &textAttributes) {
        ...
        maybeThrow(NativeNodeApi::getInstance()->setAttribute(m_nodeHandle, NODE_TEXT_INPUT_PLACEHOLDER_FONT, &item));
    }
    ...
    void TextInputNode::setCaretColor(facebook::react::SharedColor const &color) {
        ...
        maybeThrow(NativeNodeApi::getInstance()->setAttribute(m_nodeHandle, NODE_TEXT_INPUT_CARET_COLOR, &colorItem));
    }

    void TextInputNode::setMaxLength(int32_t maxLength) {
        ArkUI_NumberValue value[] = {{.i32 = maxLength}};
        ArkUI_AttributeItem item = {value, sizeof(value) / sizeof(ArkUI_NumberValue)};
        maybeThrow(NativeNodeApi::getInstance()->setAttribute(m_nodeHandle, NODE_TEXT_INPUT_MAX_LENGTH, &item));
    }

    void TextInputNode::setPlaceholder(std::string const &placeholder) {
        ArkUI_AttributeItem item = {.string = placeholder.c_str()};
        maybeThrow(NativeNodeApi::getInstance()->setAttribute(m_nodeHandle, NODE_TEXT_INPUT_PLACEHOLDER, &item));
    }

    void TextInputNode::setPlaceholderColor(facebook::react::SharedColor const &color) {
        uint32_t colorValue = *color;
        ArkUI_NumberValue preparedColorValue[] = {{.u32 = colorValue}};
        ArkUI_AttributeItem colorItem = {preparedColorValue, sizeof(preparedColorValue) / sizeof(ArkUI_NumberValue)};
        maybeThrow(
            NativeNodeApi::getInstance()->setAttribute(m_nodeHandle, NODE_TEXT_INPUT_PLACEHOLDER_COLOR, &colorItem));
    }

    std::string TextInputNode::getTextContent() {
        auto item = NativeNodeApi::getInstance()->getAttribute(m_nodeHandle, NODE_TEXT_INPUT_TEXT);
        return item->string;
    }
