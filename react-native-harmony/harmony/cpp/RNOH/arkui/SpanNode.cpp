...
SpanNode::SpanNode() : ArkUINode(NativeNodeApi::getInstance()->createNode(ArkUI_NodeType::ARKUI_NODE_SPAN)) {
}

SpanNode &SpanNode::setSpanContent(const std::string &text) {
    ArkUI_AttributeItem item = {.string = text.c_str()};
    maybeThrow(NativeNodeApi::getInstance()->setAttribute(m_nodeHandle, NODE_SPAN_CONTENT, &item));
    return *this;
}
...
SpanNode &SpanNode::setBackgroundStyle(uint32_t color) {
    ...
    ArkUI_AttributeItem item = {.value = value, .size = sizeof(value) / sizeof(ArkUI_NumberValue)};
    maybeThrow(NativeNodeApi::getInstance()->setAttribute(m_nodeHandle, NODE_SPAN_TEXT_BACKGROUND_STYLE, &item));
    return *this;
}
