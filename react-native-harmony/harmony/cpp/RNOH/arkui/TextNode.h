...
class TextNode : public ArkUINode {
  ...
  public:
    ...
    TextNode &setTextShadow(float textShadowRadius, int32_t textShadowType, uint32_t textShadowColor, float textShadowOffsetX, float textShadowOffsetY);
    TextNode &setMinFontSize(float minFontSize);
    TextNode &setMaxFontSize(float maxFontSize);
    TextNode &setTextFont(float fontSize, int32_t fontWeight = ARKUI_FONT_WEIGHT_NORMAL, int32_t fontStyle = ARKUI_FONT_STYLE_NORMAL, const std::string &fontFamily = std::string());
