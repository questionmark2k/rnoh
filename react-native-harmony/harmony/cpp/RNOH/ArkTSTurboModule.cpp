...
const std::vector<facebook::jsi::Value> convertDynamicsToJSIValues(facebook::jsi::Runtime &rt,
...
jsi::Value ArkTSTurboModule::call(jsi::Runtime &runtime, const std::string &methodName, const jsi::Value *jsiArgs,
                                  size_t argsCount) {
...
folly::dynamic ArkTSTurboModule::callSync(const std::string &methodName, std::vector<IntermediaryArg> args) {
    ...
    m_ctx.taskExecutor->runSyncTask(TaskThread::MAIN, [ctx = m_ctx, &methodName, &args, &result]() {
        ArkJS arkJs(ctx.env);
...
void rnoh::ArkTSTurboModule::scheduleCall(facebook::jsi::Runtime &runtime, const std::string &methodName,
                                          const facebook::jsi::Value *jsiArgs, size_t argsCount) {
    if (!m_ctx.arkTsTurboModuleInstanceRef) {
        auto errorMsg = "Couldn't find turbo module '" + name_ +
                        "' on ArkUI side. Did you link RNPackage that provides this turbo module?";
        LOG(FATAL) << errorMsg;
    ...
    m_ctx.taskExecutor->runTask(TaskThread::MAIN,
                                [ctx = m_ctx, name = name_, methodName, args = std::move(args), &runtime]() {
                                    try {
                                        ArkJS arkJs(ctx.env);
                                        auto napiArgs = arkJs.convertIntermediaryValuesToNapiValues(args);
                                        auto napiTurboModuleObject = arkJs.getObject(ctx.arkTsTurboModuleInstanceRef);
                                        ...
                                        LOG(ERROR) << "Exception thrown while calling " << name
                                                   << " TurboModule method " << methodName << ": " << e.what();
                                    }
                                });
}

// calls an async TurboModule method and returns a Promise
jsi::Value ArkTSTurboModule::callAsync(jsi::Runtime &runtime, const std::string &methodName, const jsi::Value *jsiArgs,
                                       size_t argsCount) {
    if (!m_ctx.arkTsTurboModuleInstanceRef) {
        auto errorMsg = "Couldn't find turbo module '" + name_ +
                        "' on ArkUI side. Did you link RNPackage that provides this turbo module?";
    ...
    try {
        m_ctx.taskExecutor->runSyncTask(
            TaskThread::MAIN, [ctx = m_ctx, name = name_, &methodName, &args, &runtime, &napiResultRef]() {
                ArkJS arkJs(ctx.env);
                auto napiArgs = arkJs.convertIntermediaryValuesToNapiValues(args);
                auto napiTurboModuleObject = arkJs.getObject(ctx.arkTsTurboModuleInstanceRef);
        ...
        return react::createPromiseAsJSIValue(runtime, [ctx = m_ctx, message = e.what()](auto &rt, auto jsiPromise) {
...
std::vector<IntermediaryArg> ArkTSTurboModule::convertJSIValuesToIntermediaryValues(
    jsi::Runtime &runtime, std::shared_ptr<react::CallInvoker> jsInvoker, const jsi::Value *jsiArgs, size_t argsCount) {
        ...
        if (jsiArgs[argIdx].isObject()) {
            ...
            if (obj.isFunction(runtime)) {
                args[argIdx] = createIntermediaryCallback(
                    react::CallbackWrapper::createWeak(std::move(obj.getFunction(runtime)), runtime, jsInvoker));
                continue;
            }
        }
...
IntermediaryCallback createIntermediaryCallback(std::weak_ptr<react::CallbackWrapper> weakCbCtx) {
    return std::function([weakCbCtx = std::move(weakCbCtx)](std::vector<folly::dynamic> cbArgs) -> void {
        ...
        cbCtx->jsInvoker().invokeAsync([weakCbCtx2 = std::move(weakCbCtx), callbackArgs = std::move(cbArgs)]() {
...
const std::vector<jsi::Value> convertDynamicsToJSIValues(jsi::Runtime &rt,
                                                         const std::vector<folly::dynamic> &dynamics) {
...
std::string preparePromiseRejectionResult(const std::vector<folly::dynamic> args) {
    ...
    throw std::invalid_argument("The type of argument provided `reject` must be string or contain a string 'message' "
